...
\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.3]{figures/flowchart.jpg}
...
$$\eta:0\longrightarrow X \longrightarrow Y \longrightarrow S \longrightarrow 0$$
where $\Img \Psi$ has an affine paving, then $\Gr(\Phi(Y))$ also have an affine paving by Theorem \ref{thm:main2}. Moreover, $\Img \Psi_{f,g}$ can be written as the product space when the map $X \hookrightarrow Y$ is a minimal section mono, so the affine paving problem of $\Img \Psi$ is usually easier than $\Gr(\Phi(Y))$.

\begin{center}
    \includegraphics[scale=0.45]{figures/minisecmono.jpg}
\end{center}
...
\end{lemma}
\begin{proof}
	Just observe the Auslander-Reiten quiver. The chosen minimal section monos are represented in Figure \ref{fig:minisecmono}. Notice that for the most time $\orde(-)$ is enough to guarantee the map to be a mono.
\end{proof}
	\begin{center}
		\begin{figure}[ht]
			\vspace{0cm}
...
Let $X \hookrightarrow Y$ be a minimal section mono, and $S:=Y/X$ be the quotient. Then we have the short exact sequence 
$$\eta: 0 \longrightarrow X \longrightarrow Y \longrightarrow S \longrightarrow 0$$
...
\end{lemma}
\begin{proof}
Since every indecomposable representation of Dynkin quiver is a brick, we get $[X,X]=[Y,Y]=1$ and $[X,X]^1=[Y,Y]^1=0$. By the definition of minimal section mono, we get
...
Now we analyse every case in Figure \ref{fig:minisecmono}, i.e. prove Claim \ref{claim:bigorder} by cases. For convenience we omit subscriptions which indicate the dimension vectors. 
...
By observation of  Figure \ref{fig:minisecmono}, $\orde(S)=\orde(Y)-\orde(X)$ is smaller or equal to $2$, so by Lemma \ref{lem:smallvecdim} $\Gr(\Phi(S))$ has the affine paving property. Let $Y_1:=X$, $X_1:=X_S$, $S_1:=Y_1/X_1$, we again use Theorem \ref{thm:main2} to get morphism
\begin{equation*}
\begin{aligned}

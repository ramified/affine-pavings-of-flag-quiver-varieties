...
When $X=P(k)$ is projective and $Y=P(j)$ is projective, it is clear that $X \longrightarrow Y$ is irreducible, and one can check by hand that $[S,X/E]^1 = [S(j), S(k)]^1=1$, so $X_S \cong E$.
...
Readers should easily read the short exact sequences from the given maps. For a short exact sequence 
$$0\longrightarrow M \longrightarrow N \longrightarrow N/M \longrightarrow 0,$$
one can get (Zariski) locally-trivial affine maps 
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
In the examples that follow, the chosen short exact sequences are omitted for clarity.
...
\begin{eg}
We verify that $\Gr(Y_2)$ is equipped with an affine paving. Referring to Figure \ref{fig:h3}, where $Y=Y_2$ and $A=X_S$, we obtain the map 
$$\Gr(Y) \longrightarrow \Gr(X) \shorttimes \Gr(Y/X) \oder U(X,A).$$

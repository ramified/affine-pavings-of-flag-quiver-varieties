...
Cellular decompositions have been used in topology for a long time as a tool in computing cohomology, while paving by affine spaces offers us a weaker substitution of the cellular decomposition. We say that an complex algebraic variety $X$ has an affine paving if $X$ has a filtration
...
Affine pavings have been constructed in many cases, such as Grassmannians, flag varieties, as well as certain Springer fibers, quiver Grassmannians or quiver flag varieties. This article focuses on the case of (strict) quiver flag varieties where the quiver is of Dynkin type or affine type, as follows:\footnote{Here $Q$ is a quiver of Dynkin or affine type(without loops), and $X\in \Rep(Q)$ is an quiver representation.}
\begin{equation*}
\begin{aligned}
...
We will prove key Theorems \ref{thm:main1} and \ref{thm:main2} in Section \ref{sec:mainthm}, implying that there are affine pavings for the quiver partial flag variety of Dynkin type in Section \ref{thm:Dynkincase}, see Figure \ref{fig:flowchart}. The difficulty in case of type $\tilde{E}$ is shown in Section \ref{sec:affine}. We refrain from giving a more detailed introduction here

...
Affine pavings imply nice properties about the cohomology of varieties, as\todo{, for exmaple the...} for example the vanishing of cohomology in odd degrees. For other properties see \cite[1.7]{de1988homology}.
...
Let $X,S \in \rep(Q)$ and $V \subseteq \Phi(X), W \subseteq \Phi(S)$, $T \in \Mod(R)$. Then\todo{Then, } $\Ext^2_{R}(W,T)=0$ and $\Ext^2_{R}(T,\Phi(X)/V)=0$.
\end{lemma}
\begin{proof}
...
\begin{lemma}[{Follows \cite[Lemma 36]{irelli2019cell}, proof is exactly the same}]\label{lem:descriptionofX_S}\todo{, with the same proof}

...
Fix a quiver $Q$. A non-zero representation $M \in \Rep(Q)$ is called indecomposable if $M$ can not be written as a direct sum of two non-zero subrepresentations.
\end{defn}
...
\end{defn}
Minimal sectional mono can also be clearly seen from the Auslander–Reiten quiver, and we can check if a sectional morphism is mono by comparing the dimension vectors. In the case of Example ???, a non-zero morphism from $A$ to $B$ is a minimal sectional mono while a non-zero morphism from $C$ to $D$ is not, since a sectional morphism from $E$ to $D$ is also injective.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

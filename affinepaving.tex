...
\section{Proof of Claim \ref{claim:bigorder}}
...
$$\eta:0\longrightarrow X \longrightarrow Y \longrightarrow S \longrightarrow 0$$
where $\Img \Psi$ has an affine paving, then $\Gr(\Phi(Y))$ also have an affine paving by Theorem \ref{thm:main2}. Moreover, $\Img \Psi_{f,g}$ can be written as the product space when the map $X \hookrightarrow Y$ is a minimal sectional mono, so the affine paving problem of $\Img \Psi$ is usually easier than $\Gr(\Phi(Y))$.

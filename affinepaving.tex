...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item $Q_d$ case: there is one arrow from $(i,r)$ to $(j,r)$ \textcolor{black}{for any $r \in \{1,\ldots,d\}$;}
			\item $Q_{d,str}$ case: there is one arrow from $(i,r)$ to $(j,r-1)$  \textcolor{black}{for any $r \in \{1,\ldots,d-1\}$;}			
		\end{itemize}
	\end{itemize}
\end{itemize}
\begin{eg}
...
\begin{center}
	...
	\end{figure}
\end{center}
and define the canonical functor $\Phi:\Rep(Q) \longrightarrow \Mod(R) \textcolor{black}{\subseteq \Rep(Q_d \text{ or } Q_{d,str})}$ as follows:
\begin{itemize}
\item $\left(\Phi(X)\right)_{(i,r)}:=X_i$;
\item $\left(\Phi(X)\right)_{(i,r) \rightarrow (i,r+1)}:=\Id_{X_i}$;
\item $\left(\Phi(X)\right)_{(i,r) \rightarrow (j,-)}:=X_{i \rightarrow j} \textcolor{black}{ \quad(i \rightarrow j)}$;
\end{itemize}

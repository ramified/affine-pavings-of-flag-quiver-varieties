...
\end{defn}
\begin{remark}
Let $f$ be a sectional morphism. If the underlying quiver $Q$ is a Dynkin/affine quiver without oriented cycles and is not a Kronecker quiver, then $X_0,\ldots,X_t$ are uniquely determined, and $f_1,\ldots, f_t$ are unique up to constant.
\end{remark}
\begin{lemma}
...
With Proposition \ref{prop:conclusion_for_combinatorics} in hand, we prove Claim \ref{claim:bigorder} by cases, which turns out to be a pure combinatorial problem. 
